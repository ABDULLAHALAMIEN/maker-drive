...
namespace makerDrive {
    ...
    export function isPowerOn(): boolean {
        if (makerDrive.i2cRead(REG_ADD_PWR_STATE) != 0) {
            return true;
        }
        else {
            return false;
    ...
    export function isLowBatt(): boolean {
        if (makerDrive.i2cRead(REG_ADD_LB_STATE) != 0) {
            return true;
        }
        else {
            return false;
    ...
    export function isOvervoltage(): boolean {
        if (makerDrive.i2cRead(REG_ADD_OV_STATE) != 0) {
            return true;
        }
        else {
            return false;
    ...
    export function readVin(): number {
        return (makerDrive.i2cRead(REG_ADD_VIN) / 10);
    }
}
